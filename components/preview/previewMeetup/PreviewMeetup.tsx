...
const PreviewMeetup = (props: PreviewThemeProps | PreviewMeetupProps): JSX.Element => {
    ...
    const loadAllVotedUsers = async () => {
        ...
        const receivedMeetups: Array<User> = []
    ...
    return (
        <article style={{ width: "100%" }}>
            {type === 'theme' ?
                <h5 className={styles.blockName}>{/*t('name')*/}name</h5>
                :
                <>
            ...
            {type === 'meetup' && (getMeetupDate(props.meetup) || getMeetupTime(props.meetup) || props.meetup.place) && (
                <>
                    <h5 className={styles.blockName}>{/*{t('timeAndLocation')}*/}timeAndLocation</h5>
            ...
            <h5 className={styles.blockName}>{type === 'theme' ? 'author' : 'speaker'/*t('author') : t('speaker')*/}</h5>
            <div className={styles.authorBlock}>
                <ProfileInfo user={props.meetup.author} first="avatar" userAvatarClassName={styles.userAvatar} />
            </div>
            <h5 className={styles.blockName}>{/*{t('description')}*/}description</h5>
            <div className={styles.excerptBlock}>
                <p>{props.meetup.excerpt}</p>
            </div>
            {type === 'theme' &&
                <>
                    <h5 className={styles.blockName}>{/*{t('Support')}*/}support</h5>
                    <div className={styles.supportBlock}>
                        {allVotedUsers.length === 0
                            ?
                            <p className={styles.noSupports}>{/*{t('noSupports')}*/}nosupport</p>
                            :
            ...
            <div data-ref="previewMeetup.controlButtons" className={styles.buttonsBlock}>
                <Button type="default" text={/*t('back')*/ 'back'} callback={(event) => props.onCancel(event)} className={styles.button} />
                {/*userStore.isModerator*/ true &&
                    ('onDelete' in props ? (
