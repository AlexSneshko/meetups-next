            ...
            <form className={styles.form} onSubmit={preventDefaultSubmit}>
                <div className={styles.inputs}>
                    ...
                    <div className={styles.theme}>
                        <LabeledInput onChange={setTheme} initialValue={subject} label={/*t('topic')*/ 'topic'} />
                    </div>
                    <div className={styles.dates}>
                        <DateInput id={'start'} setValue={setStart} value={start} label={/*t('start')*/ 'start'} className={styles.dateInput} />
                        <DateInput id={'finish'} setValue={setEnd} value={finish} label={/*t('ending')*/ 'ending'} className={styles.dateInput} />
                    </div>
                    <div className={styles.place}>
                        <LabeledInput onChange={setPlace} initialValue={place} label={/*t('place')*/ 'place'} />
                    </div>
                    <div className={styles.speaker}>
                        <LabeledInput onChange={setSpeaker} initialValue={author} label={/*t('speaker')*/ 'speaker'} />
                    </div>
                ...
                <div className={styles.buttonsBlock}>
                    <Button callback={onCancel} type="default" text={/*t('cancel')*/ 'cancel'} className={styles.button} />
                    <div className={styles.rightButtons}>
                        <Button callback={() => setIsPreview(true)} type="secondary" text={/*t('preview')*/ 'preview'} className={styles.button} />
                        <Button callback={(event) => onSave(combineData(), event)} type="primary" text={/*t('save')*/ 'save'} className={styles.button} />
                    </div>
                </div>
            </form>
        )
